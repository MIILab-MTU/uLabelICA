...
from skimage.morphology import skeletonize, remove_small_objects, binary_erosion, binary_dilation, binary_closing
...
def skeleton_by_python(file_path):
...
########################################################################################################################
# code for step 2
def calculate_end_and_joint_points_by_python(vessel_file_path):
    from labelme.core.graph_generation.utility import findendsjunctions
    skeleton_img = cv2.imread(vessel_file_path, cv2.IMREAD_GRAYSCALE)
    skeleton_img = np.asarray(skeleton_img / 255., dtype=np.bool)
    juncs, endcs = findendsjunctions(skeleton_img, 0)
    [rj, cj, re, ce] = juncs[0], juncs[1], endcs[0], endcs[1]
    ...
    for point in joint_points:
        adj_matrix = skeleton_img[point[0] - 1:point[0] + 2, point[1] - 1:point[1] + 2]
        degree = np.sum(adj_matrix == 1) - skeleton_img[point[0], point[1]]
...
def vessel_segment_point_python(img_path):
    from labelme.core.graph_generation.utility import findendsjunctions
    skeleton_img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
    juncs, endcs = findendsjunctions(skeleton_img, 0)
    rj, cj, re, ce = juncs[0], juncs[1], endcs[0], endcs[1]
    return rj, cj, re, ce
...
def diameter_map_python(binary_file_path):
    # Read the image
    img = cv2.imread(binary_file_path, cv2.IMREAD_GRAYSCALE)

    # Convert to binary (logical) image
    img = img > 0.5  # Assuming threshold value is 0.5, adjust it if needed

    # Skeletonize the image
    vessel_skeleton = skeletonize(img)

    # Calculate the Euclidean distance transform of the binary image
    vessel_edtimage = distance_transform_edt(img)

    # Calculate the diameter map
...
def get_spacing(pixel_spacing_csv_path, patient_name):
    if os.path.isfile(pixel_spacing_csv_path):
        ...
        if df.shape[0] > 0:
            ...
            warnings.warn("[!] cannot find information in pixel_spacing.csv!")
            return 0.3
    else:
        warnings.warn("[!] pixel_spacing.csv doesn't exist!")
        return 0.3
...
########################################################################################################################
# code for step 4
def __add_adjacent_points(coord, binary_image, adj_size=1):
    if adj_size <= 0:
        return []
    r_points = []
