...
def visualize_semantic_image(vessel_infos, original_image, semantic_mapping, save_path, centerline=True):
    processed_semantic_vessel = np.zeros([original_image.shape[0], original_image.shape[1], 3])
    for vessel_object in vessel_infos:
        if centerline:
            processed_semantic_vessel[:, :, 0][vessel_object.vessel_centerline == 1] = semantic_mapping[vessel_object.vessel_class][0]
            processed_semantic_vessel[:, :, 1][vessel_object.vessel_centerline == 1] = semantic_mapping[vessel_object.vessel_class][1]
            processed_semantic_vessel[:, :, 2][vessel_object.vessel_centerline == 1] = semantic_mapping[vessel_object.vessel_class][2]
        else:
...
def visualize_semantic_image2(vessel_infos, original_image, semantic_mapping, save_path, centerline=False):
    ...
    for vessel_info in vessel_infos:
        ...
        if centerline:
            if vessel_info.vessel_class not in assigned_labels:
                plt.scatter(np.where(vessel_info.vessel_centerline==1)[1], np.where(vessel_info.vessel_centerline==1)[0],
...
def visualize_binary_image(binary_images, alphas, save_path=None):
    fig, ax = plt.subplots()
...
def visualize_artery_tree_with_points(skeleton_image, end_nodes, joint_nodes, save_path):
    fig, ax = plt.subplots()
    plt.axis('off')
