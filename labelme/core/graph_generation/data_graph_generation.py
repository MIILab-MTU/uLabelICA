...
from labelme.core.graph_generation.data_processing import skeleton_by_matlab, remove_isolated_segment, \
    calculate_end_and_joint_points_by_matlab, diameter_map, get_spacing
from labelme.core.graph_generation.data_processing import __generate_vessel_binary_map, __interpolate_centerline_points, \
...
def process_single_patient(data_dir,
    ...
    semantic_image, binary_image = generate_images(semantic_files_path,
                                                   original_image.shape[0],
                                                   constants.color_map,
                                                   constants.artery_keep_dict_tw) # TODO adjust if needed
    ...
    #merge_nodes(graph, joint_nodes, distance_threshold=centerline_length_threshold)
    dist_map = diameter_map(os.path.join(save_dir, f"{patient_name}_binary_image.png"), eng)
    graph = filter_vessel(graph, dist_map, pixel_spacing,
                          radius_threshold=radius_threshold,
                          centerline_length_threshold=centerline_length_threshold)
    # nxg.visualize_networkx_graph(graph, skeleton_image,
    #                              save_path=os.path.join(save_dir, f"{patient_name}_step3-2_filtered_graph_networkx.png"))
    ...
    # graph = assign_class_on_graph(graph, semantic_image, constants.semantic_mapping)
    # nxg.visualize_semantic_image(graph, original_image, constants.semantic_mapping,
...
def data_graph_generation(data_path='/home/weihuazhou/Desktop/automated-labelme/labelme/data',
                         save_dir='/home/weihuazhou/Desktop/automated-labelme/labelme/data/processed',
                         centerline_length_threshold=8,
                         radius_threshold=0.5,
                         cardiac_pattern='LCA',
                         num_threads=1,
                         patient_name='12345678_11'):
...
#data_graph_generation(patient_name='18655313_15')